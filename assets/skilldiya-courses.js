(function(){
  ...
  function renderRecommendationSection(target,matches,careerName,careerId){
    ...
    const explorationParam=activeExplorationIdFromUrl();
    const explorationLinkParam=explorationParam?`&amp;explorationId=${encodeURIComponent(explorationParam)}`:'';
    const cards=(arr,type)=>arr.map(c=>`<article class="course-card compact"><div class="course-card-top"><span class="tag">${esc(c.domain)}</span><span class="course-status">${esc(c.status)}</span></div><h3>${esc(c.title)}</h3><p>${esc(c.description)}</p>${matchReason(type,c)?`<p class="course-match-reason">${matchReason(type,c)}</p>`:''}<div class="course-meta"><span>${esc(c.level)}</span><span>${esc(c.duration)}</span><span>${esc(c.format)}</span></div><div class="course-actions"><a class="btn btn-secondary" href="course.html?id=${encodeURIComponent(c.id)}&amp;careerId=${encodeURIComponent(careerId||'')}&amp;careerName=${encodeURIComponent(name)}${explorationLinkParam}">View Course</a><button class="btn btn-primary" data-course-interest="${esc(c.id)}" data-career-id="${esc(careerId||'')}" data-career-name="${esc(name)}" type="button">Register Interest</button></div></article>`).join('');
