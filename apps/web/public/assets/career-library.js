  ...
  function optionAccordion(items){
    return arr(items).map(item=>{
      ...
      <details class="career-option">
        <summary><span>${esc(title)}</span><span class="career-option-chevron">⌄</span></summary>
        <div>
          <p>${esc(item?.description || '')}</p>
          ${title ? `<button type="button" class="btn btn-primary career-related-asana" data-career-related-role="${esc(title)}">Use this as my target →</button>` : ''}
        </div>
      </details>`;
    }).join('');
  }
